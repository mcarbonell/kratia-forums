...
export default function ProposeNewForumPage() {
  ...
  const onSubmit: SubmitHandler<ProposeForumFormData> = async (data) => {
    ...
    const votationDescription = `Proposal to create a new forum: "${data.proposedForumName}" in category "${categories.find(c=>c.id === data.proposedForumCategoryId)?.name}". Justification: ${data.justification.substring(0,100)}...`;
...
**Proposed Category:** ${categories.find(c=>c.id === data.proposedForumCategoryId)?.name || 'Unknown'}

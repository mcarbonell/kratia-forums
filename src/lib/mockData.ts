...
const findUser = (userId: string): User => { // Return type User, not User | undefined
    ...
    if (!user) {
        ...
        return { id: 'unknown', username: 'Unknown User', email: '', avatarUrl: '', status: 'active', canVote: false };
    }
    return user;
};
...
export const mockVotations: Votation[] = [
  ...
  /*
  {
    id: 'votation1_example',
    title: 'Example Votation: Change forum name',
    description: 'Proposal to change the name of Kratia Forums to Awesome Kratia.',
    justification: 'Awesome Kratia sounds more engaging for new users.',
    proposerId: findUser('user1').id,
    proposerUsername: findUser('user1').username,
    type: 'rule_change',
    createdAt: '2023-06-01T09:00:00Z',
    deadline: new Date(Date.now() + KRATIA_CONFIG.VOTATION_DURATION_DAYS * 24 * 60 * 60 * 1000).toISOString(),
    status: 'active',
    options: { for: 5, against: 1, abstain: 0 },
    voters: { [findUser('user2').id]: 'for', [findUser('user3').id]: 'against' },
    totalVotesCast: 6,
    quorumRequired: KRATIA_CONFIG.VOTATION_QUORUM_MIN_PARTICIPANTS,
    relatedThreadId: 'votation1_thread_sticky', // Example, should point to a real thread
  },
  */
];

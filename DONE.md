...
- **Content Creation & Interaction:**
    ...
    - Users can add non-binding polls to the first post of a new thread (poll data now stored on the Thread document).
...
- **Firebase Authentication (Email/Password):**
    - User registration via Email/Password with **email verification**. (Email sent, verification checked on next login, then admission request created).
...
- **User Profile Management:**
    ...
    - Users can edit their own profiles (username, location, about me, notification preferences, avatar upload).
...
- **Karma System:**
    - Karma calculated based on: total posts made by user, total reactions received by user, total posts in threads started by the user.
...
- **Votation Outcomes & Execution:**
    ...
    - **Automatic execution of passed votations:**
        - Sanctions: Target user's status updated to `sanctioned` with an `sanctionEndDate` (hardcoded to 1 day for now).
...
- **Agora UI:**
    *   Dedicated `/agora` page listing votation threads, sorted by active then closed, with visual distinction.
...
- **User Preferences:**
    - Users can configure (enable/disable web notifications) for implemented notification types via their profile edit page.
    - The system respects these preferences when creating notifications for replies and votation conclusions (proposer & participant) and post reactions.

## VI. Private Messaging System
- **Phase 1 (DONE):** Ability to send a private message from a user's profile (via dialog). Notification for new PM (respecting user preferences).
...
- **Translated Static UI (Extensive Coverage):** Header, Footer, Homepage, Authentication pages (Login, Signup, Forgot Password, Confirm, Sanctioned), Admin Panel pages (main, create/edit for categories, forums, users), User Profile pages (view, edit), Agora pages (main, propose constitution, propose new forum), Forum/Thread/Post pages & components (UI elements, buttons, titles, messages, tooltips, toasts, poll elements), Notification page, Messages page (list and conversation view), Privacy Policy page (UI elements), Main Layout Title.
- **Language Switcher:** Implemented a dropdown in the Header to allow users to switch between English and Spanish.
- **Date Formatting:** Dates and relative time strings (e.g., "hace 2 horas", "about 2 hours ago") are localized using `date-fns` and `i18next`.
...
- **Bug Fixing & Refinements:**
    *   Resolved numerous routing issues (404s, dynamic route conflicts for proposals, Agora navigation consistency).
    *   Fixed Next.js build errors (font loading, SWC issues, module not found, i18n config).
    *   Addressed React hook rule violations and syntax errors.
    *   Corrected data consistency issues in Firestore seeding and updates (e.g., user roles, status, poll data location, sanction application).
    *   Fixed UI inconsistencies (e.g., login header update, back button navigation, duplicated post counts, responsive video embeds).
    *   Improved error handling and user feedback with toast notifications and specific error messages.
    *   Resolved issues with duplicate notification creation.
    *   Optimized data fetching and state management in various components to prevent loops (e.g., `ThreadPage`, `SanctionCheckWrapper`).
    *   Ensured Firebase Storage rules and Firestore security rules were considered and iterated upon for development.
    *   Handled i18n setup complexities with App Router, resolving 404s and ensuring translations load.
